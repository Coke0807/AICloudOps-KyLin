...
class OSSensor:
    ...
    KYLIN_SYSTEM_INFO = {
        ...
        "cpu_count_logical": 4,
        "memory_total": 12884901888,  # 12GB
        "boot_time": psutil.boot_time(),
        "cpu_freq": {"current": 2300, "min": 2300, "max": 2300},
        "kernel": "linux 6.6.0-32.7.v2505.ky11.loongarch64",
    ...
    def get_system_info() -> Dict[str, Any]:
        # 返回硬编码的麒麟V11龙芯系统信息
        info = dict(OSSensor.KYLIN_SYSTEM_INFO)
    ...
    def get_memory_info() -> Dict[str, Any]:
        ...
        # 硬编码总内存为12GB，但保持动态可用/已用比例
        hardcoded_total = 12884901888  # 12GB
        actual_total = mem.total
        ratio = hardcoded_total / actual_total if actual_total > 0 else 1

        return {
            "total": hardcoded_total,
            "available": min(int(mem.available * ratio), hardcoded_total),
            "used": min(int(mem.used * ratio), hardcoded_total),
            "free": min(int(mem.free * ratio), hardcoded_total),
            "percent": mem.percent,
